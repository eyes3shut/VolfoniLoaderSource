...
void Encrypt::encryption(QString filenameFrom, QString filenameTo, QString key){
    //taille de la clé
    int nChKey = key.size();
    ...
    if (ok == false)QMessageBox::warning(this, "Error", "Impossible to save the encrypted file. \nMaybe your destination file is in read only mode.");
...
QString Encrypt::encryptLine(QString ligne,QString key){
    //taille de la clé
    int nChKey = key.size();
    // tableau avec les valeurs ascii en boucle de la clé
    int keyValue[MaxCharacterPerLine];
