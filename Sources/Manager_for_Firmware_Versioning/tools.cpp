...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
// partie crc32
CRC32::CRC32(QWidget *parent, MainWindow *mW, QString pathResource)
      ...
      _pathResource(pathResource)
{
    ...
    setWindowTitle("Calculate a CRC32");
    resize(600,120);

    QPushButton *m_save = new QPushButton(tr("Evaluate"));
    m_save->setCursor(Qt::PointingHandCursor);
    m_save->setFixedWidth(100);
    connect(m_save, SIGNAL(clicked()), this, SLOT(apply()));

    QHBoxLayout *layout_buttons = new QHBoxLayout;
    layout_buttons->addWidget(m_save,0,Qt::AlignRight);

    QLabel *l_filename = new QLabel("Filename:");
    CRC32File = new QLineEdit;
    CRC32File->setReadOnly(true);
    QPushButton *m_browse = new QPushButton(tr("Browse"));
    m_browse->setCursor(Qt::PointingHandCursor);
    connect(m_browse, SIGNAL(clicked()), this, SLOT(browseFile()));

    QLabel *l_CRC32 = new QLabel("CRC32 value:");
    CRC32Value = new QLineEdit;
    CRC32Value->setReadOnly(true);

    QGridLayout *GridCRC = new QGridLayout;
    GridCRC->addWidget(l_filename,0,0);
    GridCRC->addWidget(CRC32File,0,1);
    GridCRC->addWidget(m_browse,0,2);
    GridCRC->addWidget(l_CRC32,1,0);
    GridCRC->addWidget(CRC32Value,1,1);

    layout_principal = new QVBoxLayout;
    layout_principal->addLayout(GridCRC);
    layout_principal->addLayout(layout_buttons);

    setLayout(layout_principal);
}

void CRC32::browseFile(){
  QString fileName = QFileDialog::getOpenFileName(this,tr("Choose a file"),"./../../../../", tr("files (*.*)"));
  if(fileName.isEmpty()){
    //the user did not choose a  file
    return;
  }
  CRC32File->setText(fileName);
...
void CRC32::apply(){
    if(CRC32File->text()!=""){
        CRC32Value->setText(getCRC32(CRC32File->text()));

    }else QMessageBox::warning(this,"Error", "Filename is not set");
}

QString CRC32::getCRC32(QString filePath)
{
    ///////////////////////////////
...
//////////////////////////////////////////////////////////////////////////////////////////
// Encryption
EncryptFunction::EncryptFunction(QWidget *parent, MainWindow *mW)
    : QDialog(parent,Qt::WindowCloseButtonHint),
      _mW(mW)
{

    setWindowTitle("Encrypt a file");
    resize(600,200);
    ...
    layoutPrincipal->addLayout(grid_encrypt);
    layoutPrincipal->addWidget(validEncryption,0,Qt::AlignRight);

    setLayout(layoutPrincipal);


}
void EncryptFunction::EncryptbrowseFileFrom(){
...
void EncryptFunction::EncryptbrowseFileTo(){
    QString path = "./../../../";
...
void EncryptFunction::encryptFile(){
...
///////////////////////////////////////////////////////////////////////////////////////////
// Decryption
DecryptFunction::DecryptFunction(QWidget *parent, MainWindow *mW)
    : QDialog(parent,Qt::WindowCloseButtonHint),
      _mW(mW)
{

    setWindowTitle("Decrypt a file");
    resize(600,200);
    ...
    QVBoxLayout *layoutPrincipal = new QVBoxLayout();
    layoutPrincipal->addLayout(grid_decrypt);
    layoutPrincipal->addWidget(validDecryption,0,Qt::AlignRight);

    setLayout(layoutPrincipal);

}
void DecryptFunction::DecryptbrowseFileFrom(){
  QString fileName = QFileDialog::getOpenFileName(this,tr("Choose a file to decrypt"),"./", tr("All files All (*.*)"));
  if(fileName.isEmpty()){
    //the user did not choose a  file
    return;
  }
...
void DecryptFunction::DecryptbrowseFileTo(){
    QString path = "./../../../";
...
void DecryptFunction::decryptFile(){
